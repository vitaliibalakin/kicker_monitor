...
class KickerApp(object):
    ...
    def adc200_kkr_default(self):
        ...
        if self.chan_adc200_numpts_2.val == 424:
            pass
        else:
            self.chan_adc200_numpts_2.setValue(424)

        if self.chan_adc200_timing_1.val == 0:
            pass
        else:
            self.chan_adc200_timing_1.setValue(0)

        if self.chan_adc200_timing_2.val == 0:
            pass
        else:
            self.chan_adc200_timing_2.setValue(0)

        if self.chan_adc200_frq_div_1.val == 0:
            pass
        else:
            self.chan_adc200_frq_div_1.setValue(0)

        if self.chan_adc200_frq_div_2.val == 0:
            pass
        else:
            self.chan_adc200_frq_div_2.setValue(0)

        if self.chan_adc200_range_1_1.val == 1:
            pass
        else:
            self.chan_adc200_range_1_1.setValue(1)

        if self.chan_adc200_range_1_2.val == 1:
            pass
        else:
            self.chan_adc200_range_1_2.setValue(1)

        if self.chan_adc200_range_2_1.val == 1:
            pass
        else:
            self.chan_adc200_range_2_1.setValue(1)

        if self.chan_adc200_range_2_2.val == 1:
            pass
        else:
    ...
    def data_proc(self, chan):
        ...
        self.expanding_data(t_data_good, u_data_good, u_data)
        self.correlation(name, ic_mode, t_data_good, u_data_good, u_data)
    ...
    def correlation(self, name, ic_mode, t_data_good, u_data_good, u_data):
        corr = np.correlate(self.ki_amp_c, self.ki_amp_g, 'same')
        delta_t = (corr.argmax() - (corr.__len__() / 2)) * self.STEP
        if abs(delta_t) < 20:
            gaussfit = lambda p, x: p[0] * np.exp(-(((x - p[1]) / p[2]) ** 2) / 2) + p[3]                           #sygnal peak and ampl
            ...
            self.dict_t_peak[name + ic_mode].setValue(round(p1[1], 0))
    ...
    def upload_good_chans(self):
        ...
        list_TC = self.list_TC
        list_DT = self.list_DT
        if self.chan_ic_mode.val[0] == 'p':
            start = 0
            end = 4
    ...
    def sigma_proc(self, delta_t, name, ic_mode):
        hist_range = self.hist_range
        bins_num = self.bins_num

        delta_t_arr = self.dict_delta_arr[name + ic_mode].val
        ...
        self.dict_sigma_t[name + ic_mode].setValue(round(sigma, 1))
        self.dict_delta_arr[name + ic_mode].setValue(delta_t_arr)
        ...
        if delta_t_arr.__len__() > 3:
            ...
            self.dict_hist[name + ic_mode][0].setValue(ppn_x)
            self.dict_hist[name + ic_mode][1].setValue(ppn_y)
            self.dict_hist[name + ic_mode][2].setValue(ppn_x_1)
            self.dict_hist[name + ic_mode][3].setValue(ppn_y_1)
            self.dict_hist[name + ic_mode][4].setValue(ppn_x_2)
            self.dict_hist[name + ic_mode][5].setValue(ppn_y_2)
            self.dict_hist[name + ic_mode][6].setValue(ppn_x_3)
